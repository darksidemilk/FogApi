...
.OUTPUTS
    Hashtable
#>
function Get-FogHttpSignedHeader {
    param(
        [string]$Method,
    ...
    $httpSigningConfiguration = Get-FogConfigurationHttpSigning
    $Digest = $null

    #get the body digest
    $bodyHash = Get-FogStringHash -String $Body -HashName $httpSigningConfiguration.HashAlgorithm
    ...
    foreach ($headerItem in $httpSigningConfiguration.HttpSigningHeader) {
        ...
        if ($headerItem -eq $HEADER_REQUEST_TARGET) {
            ...
            $created = Get-FogUnixTime -Date $dateTime -TotalTime TotalSeconds
            ...
            $expireEpocTime = Get-FogUnixTime -Date $expire -TotalTime TotalSeconds
    ...
    $signatureHashString = Get-FogStringHash -String $headerValuesString -HashName $httpSigningConfiguration.HashAlgorithm

    #Gets the Key type to select the correct signing algorithm
    $KeyType = Get-FogKeyTypeFromFile -KeyFilePath $httpSigningConfiguration.KeyFilePath

    if ($keyType -eq "RSA") {
        $headerSignatureStr = Get-FogRSASignature -PrivateKeyFilePath $httpSigningConfiguration.KeyFilePath `
        ...
        $headerSignatureStr = Get-FogECDSASignature -ECKeyFilePath $httpSigningConfiguration.KeyFilePath `
            -DataToSign $signatureHashString `
            -HashAlgorithmName $httpSigningConfiguration.HashAlgorithm `
            -KeyPassPhrase $httpSigningConfiguration.KeyPassPhrase
    }
    #Deprecated
    <#$cryptographicScheme = Get-FogCryptographicScheme -SigningAlgorithm $httpSigningConfiguration.SigningAlgorithm `
...
function Get-FogRSASignature {
    ...
    try {
        ...
        if ($PSVersionTable.PSVersion.Major -ge 7) {
            ...
            $rsa_provider_path = Join-Path -Path $PSScriptRoot -ChildPath "FogRSAEncryptionProvider.cs"
...
.OUTPUTS
    Base64String
#>
function Get-FogECDSASignature {
    param(
...
function Get-FogCryptographicScheme {
    param(
        [Parameter(Mandatory = $true)]
        [string]$SigningAlgorithm,
        [Parameter(Mandatory = $true)]
...
String
#>
function Get-FogKeyTypeFromFile {
    param(
        [Parameter(Mandatory = $true)]
        [string]$KeyFilePath
    )
